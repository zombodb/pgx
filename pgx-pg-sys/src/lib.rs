...
mod pgptr;
pub use pgptr::{IntoPgPtr, New, PgPtr};
...
pub(crate) fn node_to_string_for_display(node: PgPtr<crate::Node>) -> String {
    // crate::nodeToString() will never return a null pointer
    let node_to_string: PgPtr<::std::os::raw::c_char> = unsafe { crate::nodeToString(node.cast()) };
    let result = node_to_string.as_ref().to_owned();
    node_to_string.free();
    result
}
...
pub trait AsPgCStr {
    fn as_pg_cstr(&self) -> PgPtr<std::os::raw::c_char>;
}

impl<'a> AsPgCStr for &'a str {
    fn as_pg_cstr(&self) -> PgPtr<std::os::raw::c_char> {
        let self_bytes = self.as_bytes();
        let slice = PgPtr::<u8>::array0(self_bytes.len() + 1);
        slice.copy_from_slice(self_bytes);
        PgPtr(slice.as_mut_ptr() as *mut std::os::raw::c_char)
    }
}
...
mod all_versions {
    ...
    #[pgx_macros::pg_guard]
    extern "C" {
        pub fn pgx_list_nth(list: PgPtr<super::List>, nth: i32) -> PgPtr<std::os::raw::c_void>;
        pub fn pgx_list_nth_int(list: PgPtr<super::List>, nth: i32) -> i32;
        pub fn pgx_list_nth_oid(list: PgPtr<super::List>, nth: i32) -> super::Oid;
        pub fn pgx_list_nth_cell(list: PgPtr<super::List>, nth: i32) -> PgPtr<super::ListCell>;
        pub fn pgx_GETSTRUCT(tuple: PgPtr<pg_sys::HeapTupleData>) -> PgPtr<std::os::raw::c_char>;
    }
    ...
    pub unsafe fn rt_fetch(
        index: super::Index,
        range_table: &PgPtr<super::List>,
    ) -> Option<&super::RangeTblEntry> {
        range_table.get_ptr(index as i32 - 1)
    }

    #[inline]
    pub fn HeapTupleHeaderGetXmin(
        htup_header: PgPtr<super::HeapTupleHeaderData>,
    ...
    #[inline]
    pub fn HeapTupleHeaderIsHeapOnly(htup_header: PgPtr<super::HeapTupleHeaderData>) -> bool {
        extern "C" {
            pub fn pgx_HeapTupleHeaderIsHeapOnly(
                htup_header: PgPtr<super::HeapTupleHeaderData>,
            ) -> bool;
        }
    ...
    #[inline]
    pub fn heap_tuple_get_struct<T>(htup: PgPtr<super::HeapTupleData>) -> *mut T {
        if htup.is_null() {
            0 as *mut T
        } else {
            unsafe { pgx_GETSTRUCT(htup).as_mut_ptr() as *mut T }
        }
    }
